...
# 89 "fir/fir.cpp"
__attribute__((sdx_kernel("krnl_fir", 0))) void krnl_fir(hls::stream<axis> &x, hls::stream<axis> &y) {
#line 13 "C:/Users/tsope/OneDrive/Documents/GitHub/HW-IP/fir/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=krnl_fir
# 89 "fir/fir.cpp"
...
#pragma HLS ARRAY_PARTITION variable=state dim=1 complete
 ...
 if (!x.empty())

  {
    ...
    VITIS_LOOP_109_1: for (unsigned int i = 0; i < 73; i++){
    ...
    if(!x_k.last){
      VITIS_LOOP_119_2: for (unsigned int i = 0; i < 73 -2; i++){
        state[i] = state[i+1] + prod[i+1];
      }
      state[73 -2] = prod[73 -1];
    }
    else {
      VITIS_LOOP_125_3: for (unsigned int i = 0; i < 73 -1; i++){
        state[i] = 0;
      }
    }
